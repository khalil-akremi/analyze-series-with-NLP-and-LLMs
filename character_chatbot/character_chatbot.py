...
class CharacterChatBot:
    ...
    CharacterChatBot with:
      - Correct TRL SFTTrainer API (uses formatting_func, not tokenizer or dataset_text_field)
      - Correct LoRA task_type (CAUSAL_LM)
      - Stable chat() using chat templates + text-generation pipeline
    ...
    def __init__(
        ...
        if huggingface_hub.repo_exists(self.model_path):
            ...
            print("Model not found on HuggingFace Hub, training a new adapter...")
    ...
    def chat(self, message: str, history: list[list[str]]):
        ...
        messages = [{"role": "system",
                     "content": "You are Walter White from Breaking Bad. Respond like him."}]
        for user_msg, assistant_msg in history:
            messages.append({"role": "user", "content": user_msg})
            messages.append({"role": "assistant", "content": assistant_msg})
        messages.append({"role": "user", "content": message})

        # Convert to single prompt using chat template
