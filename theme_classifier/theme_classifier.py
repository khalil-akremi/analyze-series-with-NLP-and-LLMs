...
class ThemeClassifier():
    ...
    def get_themes_inference(self, script):
        ...
        # Run Model on ALL batches (not just first 2)
        theme_output = self.theme_classifier(
            script_batches,  # Process all batches, not just [:2]
            self.theme_list,
            multi_label=True
        )
    ...
    def get_themes(self, dataset_path, save_path=None):  # Fixed typo: dtaset_path -> dataset_path
        ...
        # Ensure all theme columns exist, fill with 0 if missing
